...
# Dapatkan IP awam (fallback curl/wget)
MYIP=""
if command -v wget >/dev/null 2>&1; then
  MYIP="$(wget -qO- icanhazip.com 2>/dev/null || true)"
fi
if [ -z "$MYIP" ] && command -v curl >/dev/null 2>&1; then
  MYIP="$(curl -s ifconfig.me 2>/dev/null || true)"
fi

# Email domain (default boleh ditetapkan di sini jika nak)
DEFAULT_EMAIL="admin@localhost"
default_email="${default_email:-$DEFAULT_EMAIL}"

printf '\e[1;32m════════════════════════════════════════════════════════════\e[0m\n\n'
printf '   \e[1;32mPlease enter your email Domain/Cloudflare.\e[0m\n'
printf '   \e[1;31m(Press ENTER for default email)\e[0m\n'
printf '   Email : '
read -r email
if [ -z "${email:-}" ]; then
  sts="$default_email"
